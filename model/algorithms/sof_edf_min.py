...
def bucket(task, partition):
    ...
    b = task['period'] - sum(map(lambda x : task['period'] * (float(x['execution'])/x['period']), partition))
    return min(a,b)
...
def schedulable(taskset, processors, bounds, heuristic = 'first-fit'):
    ...
    for task in heavy_tasks:
        group = []
        mi = math.ceil(float(task['execution'] - task['critical']) / (task['deadline'] - task['critical']))
        if mi >= float('inf'):
            return False
