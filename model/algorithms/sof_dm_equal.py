...
def bucket(task, partition):
    util = sum(map(lambda x : float(x['execution'])/x['period'], partition))
    wcet = sum(map(lambda x : x['execution'], partition))
    utwc = sum(map(lambda x : float(x['execution'])/x['period'] * x['execution'], partition))

    a = task['deadline'] - task['deadline'] * util - wcet + utwc
    b = task['period'] - task['period'] * util
    return min(a, b) 
...
def schedulable(taskset, processors, bounds, heuristic = 'first-fit'):
    ...
    for task in heavy_tasks:
        ...
        for j in range(int(mi)):
            reservation = dict()
            reservation['execution'] = (1 + (float(task['execution'] - task['critical']) / (mi*task['critical']))) * task['critical']
            reservation['deadline'] = task['deadline']
            reservation['period'] = task['period']
            reservation['critical'] = task['critical']
            reservation['bound'] = max(mi, math.ceil(float(task['execution'])/task['critical']), bounds)
